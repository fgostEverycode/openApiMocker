...
class Server {
	...
	sendResponse(req, res, body, statusCode, headers, responseMimeType = '') {
		...
		/**
		 * Si el body (la respuesta) es una Response, trata la respuesta como un externalValue
		 */
		const hasExternalValue = !!body?.body;

		if(hasExternalValue) {
			res.set('Content-Type', body.headers.get('Content-Type'));
			res.set('Content-Length', body.headers.get('Content-Length'));
			res.writeHead(statusCode, { 'Content-Type': responseMimeType });
			const readableStream = body.body;
