...
class Path {
	...
	async getResponse(preferredStatusCode, preferredExampleName) {
		...
		const body = schema ? await ResponseGenerator.generate(schema, preferredExampleName) : null;
		return {
			statusCode: Number(statusCode),
			headers: headers && await this.generateResponseHeaders(headers),
			body,
			responseMimeType
		};
	}
	...
	async generateResponseHeaders(headersSchema) {

		const responseHeaders = {};

		for(const [headerName, headerData] of Object.entries(headersSchema))
			responseHeaders[headerName] = await ResponseGenerator.generate(headerData);
